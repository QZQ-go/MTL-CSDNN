...
def get_dataset(target_disease=None, ):
    """
    一个简单的函数实现区分训练，测试和验证集
    可以指定返回几个数据集 与 返回什么病的数据集
    """
    if target_disease:
        data_set = SingleDiseaseDataSet(target_disease)
    else:
    ...
    train_dataset, val_dataset, test_dataset = random_split(
        dataset=data_set, lengths=[train_len, val_len, test_len], generator=torch.Generator().manual_seed(0)
    )

    # 返回需要用到数据集，不需要则舍弃
    result_dict = {'train_dataset': train_dataset, 'val_dataset': val_dataset, 'test_dataset': test_dataset}
    return result_dict
...
def fetch_ds(target_disease, pos_weight, neg_weight):
    train_ds, val_ds, test_ds = get_dataset(target_disease)
    train_dl = DataLoader(dataset=train_ds,
                          batch_size=1,
                          sampler=get_over_load_sampler(train_ds, pos_weight, neg_weight))
    val_dl = DataLoader(dataset=val_ds,
                        batch_size=1,
                        sampler=get_over_load_sampler(val_ds, pos_weight, neg_weight))
    test_dl = DataLoader(dataset=test_ds,
                         batch_size=1,
                         sampler=get_over_load_sampler(test_ds, pos_weight, neg_weight))

    train_x, train_y = split_x_y_into_numpy(train_dl)
    val_x, val_y = split_x_y_into_numpy(val_dl)
    test_x, test_y = split_x_y_into_numpy(test_dl)

    train_y.resize(train_y.shape[0])
    val_y.resize(val_y.shape[0])
