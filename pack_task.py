...
def tabnet_train(trial, if_trial=True):
    ...
    model_name = 'tabnet'
    prc_file_name = f'files/{trial.number}_{model_name}_prc.csv' if if_trial else None
    metrics_file_name = f'files/{trial.number}_{model_name}_result.csv' if if_trial else f'{model_name}_result.csv'
